...
if _fs_conf.read(os.path.join('/etc/swift', 'fs.conf')):
    ...
    try:
        _account_is_volume = _fs_conf.get(
            'DEFAULT', 'account_is_volume', "yes") in TRUE_VALUES
    except (NoSectionError, NoOptionError):
        _account_is_volume = True
else:
    _mkdir_locking = False
    _use_put_mount = False
    _account_is_volume = True
...
class DiskFile(object):
    ...
    def __init__(self, mgr, dev_path, threadpool, account, container, obj,
        ...
        if _account_is_volume:
            rel_container_path = container
        else:
            rel_container_path = os.path.join(account, container)
        self._container_path = os.path.join(self._device_path,
                                            rel_container_path)
        ...
        if _use_put_mount:
            self._put_datadir = os.path.join(
                self._device_path + '_PUT', rel_container_path, self._obj_path)
        else:
